...
def buildBenchmarkValues():
    ...
    print
    "BenchmarkValues %s built" % benchmarkValues.keys()
...
class ChinaReturn():
    ...
    def analyze(self):
        ...
        for fileName in filter(lambda f: f.endswith('.xls'), os.listdir(self.__workingDir)):
            ...
            for sheetName in sheetNames:
                with ExcelLib(excelFile) as excel:
                    ...
                    print
                    'Processing %s with benchmark %s' % (sheetName, benchmark)
                    ...
                    for index, duration in enumerate(self.dayIntervals):
                        ...
                        for i in range(1, duration + 1):
                            ...
                            try:
                                ...
                                print
                                'Analyzing %s break at %d' % (sheetName, i)
                                broke = True
                                break
                        if data:
        ...
        with open(os.path.join(self.__workingDir, 'output.txt'), 'w') as outputFile:
            ...
            print
            "Days since going public %s" % self.dayIntervals
            print
            "returnRates: %s" % outputReturnRates
            print
            "alphas: %s" % outputAlphas
            print
            "relativeReturnRates: %s" % outputRelativeReturnRates
