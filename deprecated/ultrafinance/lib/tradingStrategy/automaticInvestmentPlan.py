...
def fixAmountPerPeriodWithAddtionWhenDrop(dateValues, interval, slidingWindow):
    ...
    adjustShare = sum([amount / dateValues[index].value for index in range(len(dateValues)) if
                       index >= slidingWindow and min(
                               [dateValue.value for dateValue in dateValues[index - slidingWindow:index + 1]]) ==
...
def adjustFixAmountPerPeriod(dateValues, interval, slidingWindow):
    ...
    for frame in range(len(dateValues) / interval):
        ...
        for i in range(interval):
            ...
            if index >= slidingWindow and min(
                    [dateValue.value for dateValue in dateValues[index - slidingWindow:index + 1]]) == dateValues[
                index].value:
