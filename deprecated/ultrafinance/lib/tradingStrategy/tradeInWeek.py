...
class TradeInWeek():
    ...
    def oneStrategy(self, buyWeekDate, sellWeekDate):
        ...
        with ExcelLib(self.fileName, self.sheetNumber) as excel:
            ...
            for i, stockDate in enumerate(
                    [date(int(stockDate.split('-')[0]), int(stockDate.split('-')[1]), int(stockDate.split('-')[2]))
                ...
                if buyWeekDate == stockDate.isoweekday():
                    for holdDay in range(1, 5):
                        if i + holdDay < len(stockDates):
                            ...
                            if sellWeekDate == date(int(stockDate.split('-')[0]), int(stockDate.split('-')[1]),
                                                    int(stockDate.split('-')[2])).isoweekday():
