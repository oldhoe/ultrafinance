    ...
    def validate(self, order, tick):
        ...
        if Action.BUY == order.action:
            ...
            if order.symbol not in self.__holdings:
                ...
                msg = 'Transition fails validation: share %s is not enough as %s' % (
                    order.share, self.__holdings[order.symbol][0])
            elif self.__commision > self.__cash:
                msg = 'Transition fails validation: cash %s is not enough for commission %s' % (
                    self.__cash, self.__commision)
